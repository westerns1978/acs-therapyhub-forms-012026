...
> **Status: #10b RESOLVED (2026-06-06) · #10a RESOLVED (2026-06-08).** ✅ **#10a — both static columns DROPPED** via migration `20260608_demo_week_seed_and_10a_drop` (folded into the demo-week seed); the seed writers `20260518_demo_data_marcus_pat` + `20260605_demo_data_completed_oep` were stripped of their column refs. `clients.compliance_score` is **intentionally RETAINED** (still backs the Director "Compliance Rate" — its own follow-up, NOT part of #10a). Witnessed: Marcus baseline unchanged (16/75 · 16/35 · 47/90 · $175); clean revert proven. Historical context below. The portal-determination RPC `public.my_progress()` shipped and is live (see (b)). **#10a is now UNBLOCKED — WS6 (standing groups) merged to `main` 2026-06-07 (`9093d1b`)**, so the cross-branch hold is lifted. The static `srop_hours_completed` / `total_sessions_required` columns are unread in every branch and can now be dropped (with their seed writers) as a standalone cleanup — see (a) plus the §424 / §454 follow-ups that fold into it.
