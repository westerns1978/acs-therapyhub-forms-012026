 ...
 *
 * BUILDS FIRST (added 2026-07-27). firebase.json's `predeploy` hook runs only the
 ...
 * failure is silent and reassuring, which is the worst combination. A failed
 * build now aborts before any upload — a stale dist must never reach the site.
 */
...
// 1. Build. Must succeed, or we never reach the upload.
run('building dist/ from the current working tree', 'npm', ['run', 'build']);

// 2. Deploy. The predeploy guard re-checks DEPLOY_TARGET + firebase.json.
