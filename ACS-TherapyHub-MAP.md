...
Owner: Dan Western (Gemynd). Last updated: **2026-06-06.**
...
> **Assessment spine — reg-grounded (2026-06-06).** The SATOP placement rules are grounded in **9 CSR 30-3.206 (verified Jan-2024 text)** — offense base, SROP floor, hours (OEP 10 · WIP 20 · CIP 50 · SROP 75 incl. ≥35 counseling), screening 6-mo validity, cert-within-7-days, DOR auto-notification. The rule pack is now cited to **3.206** on the corrected rules and **CIP's hours gate is now enforceable** (50-total, was `not_enforceable`). This is **not** "demo-only pending sign-off": a **licensed clinician confirms each determination at sign-off** (the engine proposes; the clinician signs the clinical act). As of **WS3** the **SROP ≥35-hour counseling** floor is **ENFORCED** from real Completed hours, and as of **WS4** the **required total** derives from the **signed determination's level** (not the static column). The completion gate now reads **both** sides from authoritative sources — *completed* from accrual, *required* from the signed determination — and **neither passes on seed**.

**Recent commit trail (this week):** Record Payment `7757725`+`428c3dc` · widen-financial `8a91dc3`+`75417bf` · retire-mock-billing `96144c7` · receipt PDF `38c5025` · Director Reports `20260605_reports_1…` + merge `f991324` · docs un-hide `e7cb41d` · WS1 migration `a68dfbd` + feature `bb3f7cb` + merge `7d5c3e5` · Clara relocation merge `d5f8b5d` · Clara avatar+pulse merge `3bef251` · **WS2** Phase 1 `0ca04ef` (ws2_1 table+RLS · ws2_2 grant-lock) · Phase 2 sign-off `8c4fa06` · Phase 3 CIMOR packet `e394a91` · merge `7611811` · **WS2.5** engine+truth-table `87e7a93` · strip `46b3877` · merge `5f052d7` · **reg-3.206** corrections `c852796` · merge `5144bb7` · **WS3** accrual migration `f743b44` + gate+UI `dacde36` · merge `d4607d8` · **WS4** required-from-determination `0ed5191` · merge `638af2a`. (Earlier WS0/WS6/WS7/WS-Billing history is in `ACS-Session-Handoff.md`.)
...
| `/sign/:docType/:id` | D/T/A · deep-link | Sign consent/plan — **save is a no-op** | MOCK |
...
| `/portal/documents/sign/:id` | Sign a doc — **save no-op** | MOCK |

**⚠ Routable but NOT production-real — never demo as working:** signature save (no-op, staff *and* portal) · ActiveSession SOAP (not persisted) · Analytics (hardcoded, hidden) · AI Documents (black-screens, hidden) · Settings OAuth (simulated) · ProgressTracking analysis (stub) · RecoveryPlanForm (seeded) · Compliance audit-log (read-only, nothing written) · relapse-risk card (hidden). *These are the day-30 landmines — the demo script must route around them.*
...
- *Nothing active.* **WS3 — categorized session-hours accrual** shipped (→ §4) — the first real slice of the online-session system of record (the *completed-hours* side feeds the gate). Next on that spine (all deferred): scheduling/rosters with **persisted** attendees + per-program group caps, Zoom **participant-report** attendance (vs. "marked complete"), iVALT-at-join. **Near-term §7:** derive the *required-total* from the signed level.
...
- **Required-total side — RESOLVED (WS4).** The gate derives the required total + SROP counseling floor from the **current signed determination's level** (`REQUIRED_HOURS_BY_LEVEL`), not the static `total_sessions_required`. The completion gate now reads **both** sides from authoritative sources — *completed* from accrual (WS3), *required* from the signed determination (WS4) — and **neither passes on seed**; no signed determination → "completion not established."
- **Displays still read static columns** (numbers, not the authoritative sources): `srop_hours_completed` **and** `total_sessions_required` in `PortalDashboard` / `ClientOverviewTab` / progress-% (`api.ts`) / `alertsService`'s 90-day plan-review heuristic; plus the seeded **"SATOP Level IV — Court mandate"** free-text descriptor on the client record (a referral note, not a computed level — the *computed* level badge in `ClientProfileHeader` reads the determination as of WS4). The **gate** reads the authoritative sources (accrual + signed determination); these displays can drift until switched. Smaller follow-on. (Surfaced WS3/WS4.)
