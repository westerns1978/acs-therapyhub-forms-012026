...
> **Assessment spine — reg-grounded (2026-06-06).** The SATOP placement rules are grounded in **9 CSR 30-3.206 (verified Jan-2024 text)** — offense base, SROP floor, hours (OEP 10 · WIP 20 · CIP 50 · SROP 75 incl. ≥35 counseling), screening 6-mo validity, cert-within-7-days, DOR auto-notification. The rule pack is now cited to **3.206** on the corrected rules and **CIP's hours gate is now enforceable** (50-total, was `not_enforceable`). This is **not** "demo-only pending sign-off": a **licensed clinician confirms each determination at sign-off** (the engine proposes; the clinician signs the clinical act). One honest gap — the **SROP ≥35-hour counseling** floor is *recorded, not enforced* until session hours are categorized (see §7).

**Recent commit trail (this week):** Record Payment `7757725`+`428c3dc` · widen-financial `8a91dc3`+`75417bf` · retire-mock-billing `96144c7` · receipt PDF `38c5025` · Director Reports `20260605_reports_1…` + merge `f991324` · docs un-hide `e7cb41d` · WS1 migration `a68dfbd` + feature `bb3f7cb` + merge `7d5c3e5` · Clara relocation merge `d5f8b5d` · Clara avatar+pulse merge `3bef251` · **WS2** Phase 1 `0ca04ef` (ws2_1 table+RLS · ws2_2 grant-lock) · Phase 2 sign-off `8c4fa06` · Phase 3 CIMOR packet `e394a91` · merge `7611811` · **WS2.5** engine+truth-table `87e7a93` · strip `46b3877` · merge `5f052d7` · **reg-3.206** corrections `c852796` · merge `5144bb7`. (Earlier WS0/WS6/WS7/WS-Billing history is in `ACS-Session-Handoff.md`.)
...
**Orphaned-but-real (exist, role-gated, no nav link):** `/compliance-assistant` (Clara co-pilot) · `/video-sessions` + green-room (**real Zoom session rows — recon this before the online-session workstream; infra may already exist**). Also: `/compliance` is missing from the desktop sidebar (mobile-drawer only).
...
- **Session hour-type categorization (`hourComponents`, currently `null`)** — the missing data model behind **every** per-category hour rule: SROP **≥35 counseling** (recorded-not-enforced as of the 3.206 corrections), CIP's removed 10/20/20 split, the comparable ≥40 counseling. **The single root blocker** for enforcing any composition requirement. (Surfaced in the 9 CSR 30-3.206 corrections.)
